...
public class GroovyGlueAdaptor {

	private final Lookup lookup;
	private final Glue glue;

	public GroovyGlueAdaptor(Lookup lookup, Glue glue) {
		this.lookup = lookup;
		this.glue = glue;
	}

	protected void addDefinition(Method method, Annotation annotation) {
		Class<? extends Annotation> annotationType = annotation.annotationType();
		if (annotationType.getAnnotation(StepDefinitionAnnotation.class) != null) {
			String expression = expression(annotation);
			glue.addStepDefinition(new GroovyStepDefinition(method, expression, lookup));

//			CachingGlueUtil.getStepDefinitions( glue).stream().filter(sd -> sd.)
...
//			glue.addParameterType(
//					new JavaParameterTypeDefinition(name, pattern, method, useForSnippets, preferForRegexMatch, useRegexpMatchAsStrongTypeHint, lookup));
...
//					new JavaDefaultDataTableEntryTransformerDefinition(method, lookup, headersToProperties, replaceWithEmptyString));
...
//			glue.addDocStringType(new JavaDocStringTypeDefinition(contentType, method, lookup));
	...
	protected static String expression(Annotation annotation) {
		try {
